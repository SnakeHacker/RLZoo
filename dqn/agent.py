...
class Replaymemory:
    ...
    def sample(self):
        ...
        batch_done_tensor = torch.tensor(np.asarray(batch_done), dtype=torch.int64).unsqueeze(-1)
...
class Agent:
    def __init__(self, n_input, n_output):
        ...
        self.online_net = DQN(n_input=n_input, n_output=n_output)
        self.target_net = DQN(n_input=n_input, n_output=n_output)
