...
import os
import time
from maddpg_agent_v1 import Agent, Critic, Actor
...
for agent_i in range(NUM_AGENT):
    agent = Agent(mem_size=MEMORY_SIZE, obs_dim=obs_dim[agent_i], state_dim=state_dim ,n_agent=NUM_AGENT ,action_dim=action_dim[agent_i], alpha=LR_ACTOR, beta=LR_CRITIC, fc1_dims=HIDDEN_DIM, fc2_dims=HIDDEN_DIM, gamma=GAMMA, tau=TAU, batch_size=BATCH_SIZE)
    agents.append(agent)
...
for episode_i in range(NUM_EPISODE):
    ...
    for step_i in range(NUM_STEP):
        ...
        multi_next_obs, multi_rewards, multi_done, multi_truncation, infos = env.step(multi_actions)
        ...
        if step_i >= NUM_STEP -1:
            multi_done = {agent_name: True for agent_name in env.agents}
        ...
        for agent_i in range(NUM_AGENT):
            agent = agents[agent_i]
            batch_obses, batch_next_obses, batch_states, batch_next_states, batch_actions, batch_rewards, batch_dones\
                = agent.replay_buffer.sample(batch_idx)

            # Single + batch
            batch_obses_tensor = torch.tensor(batch_obses,  dtype=torch.float)
            batch_next_obses_tensor = torch.tensor(batch_next_obses, dtype=torch.float)
            batch_states_tensor = torch.tensor(batch_states, dtype=torch.float)
            batch_next_states_tensor = torch.tensor(batch_next_states, dtype=torch.float)
            batch_actions_tensor = torch.tensor(batch_actions, dtype=torch.float)
            batch_rewards_tensor = torch.tensor(batch_rewards, dtype=torch.float)
            batch_dones_tensor = torch.tensor(batch_dones, dtype=torch.float)
        ...
        multi_batch_actions_tensor = torch.cat(multi_batch_actions, dim=1)
        multi_batch_next_actions_tensor = torch.cat(multi_batch_next_actions, dim=1)
        multi_batch_online_actions_tensor = torch.cat(multi_batch_online_actions, dim=1)
        ...
        if (total_step + 1) & TARGET_UPDATE_INTERVAL == 0:
            for agent_i in range(NUM_AGENT):
                ...
                multi_batch_online_actions_tensor = torch.cat(multi_batch_online_actions_list, dim=1)
    ...
    if (episode_i +1)% 200 == 0:
        env = simple_adversary_v3.parallel_env(N=2, max_cycles=NUM_STEP, continuous_action=True, render_mode="human")
    ...
    if episode_reward > highest_reward:
        highest_reward = episode_reward
        print(f"Highest reward updated {episode_i}: {round(highest_reward, 2)}")
        for agent_i in range(NUM_AGENT):
            agent = agents[agent_i]
