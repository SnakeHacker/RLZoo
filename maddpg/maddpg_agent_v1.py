...
class Critic(nn.Module):
    ...
    def save_checkpoint(self, checkpoint_file):
        torch.save(self.state_dice(), checkpoint_file)
...
class Agent():
    def __init__(self, mem_size, obs_dim, state_dim, n_agent, action_dim,
    alpha, beta, fc1_dims, fc2_dims, gamma, tau, batch_size):
        ...
        self.replay_buffer = ReplayBuffer(capacity=mem_size, obs_dim=obs_dim, state_dim=state_dim, action_dim=action_dim, batch_size=batch_size)

    def get_action(self, obs):
        single_obs = torch.tensor(data=obs, dtype=torch.float).unsqueeze(0)
        single_action = self.actor.forward(single_obs)
        noise = torch.randn(self.action_dim)*0.2
        single_action = torch.clamp(input=single_action + noise, min=0.0, max=1.0)

        return single_action.detach().numpy()[0]
